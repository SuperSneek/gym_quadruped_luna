...
class QuadrupedEnv(gym.Env):
    ...
    def hip_positions(self, frame='world') -> LegsAttr:
        ...
        joint_names = [self.robot_cfg.leg_joints['FL'][0],
            self.robot_cfg.leg_joints['FR'][0],
            self.robot_cfg.leg_joints['RL'][0],
            self.robot_cfg.leg_joints['RR'][0]]
        
        #remove suffix "_joint"
        joint_names = [name.replace('_joint', '') for name in joint_names]

        FL_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, joint_names[0])
        FR_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, joint_names[1])
        RL_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, joint_names[2])
        RR_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, joint_names[3])
    ...
    def _find_feet_model_attrs(self, feet_geom_name):
        ...
        # Get list of all geometry names in the model
        _all_geoms = [mujoco.mj_id2name(self.mjModel, i, mujoco.mjtObj.mjOBJ_GEOM) for i in range(self.mjModel.ngeom)]
        print(f"Found {len(_all_geoms)} geometries in the model:")
        for i, geom in enumerate(_all_geoms):
            print(f"  {i}: {geom}")
